...
Specifically, it first divides all texts into topics and emotions allowing for an overview of the data and, afterwords, it performs topic modeling on each sentiment or category defined by the user separately so a more in-depth analysis can be perfomred.
...
|-met                        |--mean_text                 |MEAN_TEXT                 |Columns in TEXT_DATA to compute the mean of in _Summary.csv_.|
...
|-r_classification          |--remove_classification     |REMOVE_CLASSIFICATION     |If you are just interested in a global topic classification, without wanting to divide the texts in any way for a more in-depth analysis, set this flag so only the topic modelling corresponding to all texts is performed. The report will only show the 'Summary' section.|
...
- **_Texts.csv_**: A csv file with all the texts considered in the analysis, it contains the following columns:
    ...
    + _global\_topic_: The topic a specific text belongs to according to the topic modelling performed on all texts.
...
LinguaLoupe is currently on version v.0.3.0-alpha.
...
- Add another CSV output containing GROUP_TO_COUNT_TEXT with metrics MEAN_TEXT and SUM_TEXT computed per group. 
