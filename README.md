...
|-gbc                       |--group_by_column           |GROUP_BY_COLUMN           |Columns in TEXT_DATA by which to group by when summerizing the information, the results can be found in the file _Summary.csv_ inside the output directory.|
|-mean                        |--mean_text                 |MEAN_TEXT                 |Columns in TEXT_DATA to compute the mean of in _Summary.csv_.|
|-sum                        |--sum_text                  |SUM_TEXT                  |Columns in TEXT_DATA to sum in _Summary.csv_.|
