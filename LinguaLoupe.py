...
parser.add_argument("-st", "--sum_text", help="Columns in TEXT_DATA to sum in Summary.csv.",
                    action="append", required=False, default=[])
parser.add_argument("-umap_colour", "--umap_colour", type=str, help="Column in TEXT_DATA by which the umap shown in the report will be colored by.",
                    default="emotion",
                    required=False)
...
parser.add_argument("-lang", "--language", type=str, help="The main language used in your documents, currently only 'english' is supported.", default="english", required=False)
...
if (u_col != "emotion") and (u_col not in cols_keep_text):
    print("umap_colour not in Columns_to_Keep_Text")
else:
