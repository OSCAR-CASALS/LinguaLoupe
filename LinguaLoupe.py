...
parser.add_argument("-met", "--mean_text", help="Columns in TEXT_DATA to compute the mean of in Summary.csv.",
...
parser.add_argument("-umap_colour", "--umap_colour", help="Column in TEXT_DATA by which the umap shown in the report will be colored by, this parameter can be specified more than once in case you want to generate multiple UMAPs coloured by different values.",
                    ...
                    required=False)
parser.add_argument("-col", "--Category_Column", required=False, default="emotion", type=str, help="Column by which to devide text in the report. By default it is a new column called 'emotion' created by the pipeline.")
...
parser.add_argument("-r_classification", "--remove_classification", action="store_false", help="If you are just interested in a global topic classification, without wanting to divide the texts in any way for a more in-depth analysis, set this flag so only the topic modelling corresponding to all texts is performed. The report will only show the 'Summary' section.")
...
print(f"Embedding model used in topic classification: {emodel}")
...
run_sentiment_pipeline(text_data, title, text_col, cols_keep_text, count_text_group, mean_text_cols, sum_text_cols,
                       ...
                       clean_html=c_html, include_tables=show_tables, sentiment_classification=r_classification, embedding_model_name=emodel,
                       embedding_device=edevice, group_column=gr_col, sentiment_device=sent_dev, m_topic_size_global = m_topic_size_global)
