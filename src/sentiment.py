...
def load_classification_model(language = "english", model_type="social_media", device = -1):
    '''
    ...
    if model_type == "social_media":
        ...
        if language == "english":
            ...
            classifier = pipeline("sentiment-analysis", model=model, tokenizer=tokenizer, max_length=512, truncation=True, device = device)
        ...
        if language == "english":
            classifier = pipeline("sentiment-analysis",model="siebert/sentiment-roberta-large-english", device = device)
            return classifier
